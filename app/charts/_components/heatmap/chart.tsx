...
// TODO: Make this Component to Reusable Component
export function HeatmapChart() {
  ...
  const xLabels = useMemo(() => {
    const weeks = [...new Set(data?.map((d) => d.periode))];
    return weeks;
  }, []);
  ...
  const formatPeriode = useCallback((value: number) => {
  ...
  const matrix: MatrixCategoryPoint[] = useMemo(() => {
    ...
    xLabels.forEach((periode, xIndex) => {
      yLabels.forEach((product, yIndex) => {
        const dataPoint = data?.find((item) => item.periode === periode && item.name === product);
        matrixData.push({
          x: xIndex,
          y: yIndex,
